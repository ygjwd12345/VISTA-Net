...
class EncNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False,
        ...
        self.head = EncHead([512, 1024, 2*512], self.nclass, se_loss=se_loss, attentiongraph=kwargs['attentiongraph'],
    ...
    def forward(self, x,epoch,rank):
        imsize = x.size()[2:]
        features = self.base_forward(x,epoch,rank)
        ##
        # atten_ = features[3][1:]
        # features = features[:3] + (features[3][0],)
        ##
        x = list(self.head(*features))
        #x = list(self.head(features[:4]))
        x[0] = F.upsample(x[0], imsize, **self._up_kwargs)
        if self.aux:
            auxout = self.auxlayer(features[2])
            auxout = F.upsample(auxout, imsize, **self._up_kwargs)
            x.append(auxout)
        # return tuple(x) + atten_
        return tuple(x)
...
class EncHead(nn.Module):
    def __init__(self, in_channels, out_channels, se_loss=True, attentiongraph=True, lateral=False,
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(in_channels[-1], 512, 1, bias=False),
                                   norm_layer(512),
                                   nn.ReLU(inplace=True)) if attentiongraph else \
