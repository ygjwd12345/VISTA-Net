...
class BtsModel(nn.Module):
    def __init__(self, params):
        ...
        ### scale 3
        self.AttentionGraphCondKernel_5 = AttentionGraphCondKernel([512, 1024, 2048], width=512)
        # self.multihead=multihead(scale=3,heads=4,channel=256)
    def forward(self, x, focal,rank=1):
        skip_feat = self.encoder(x)
        skip_feat[5]=self.AttentionGraphCondKernel_5(skip_feat[2],skip_feat[3],skip_feat[4],skip_feat[5],rank)
        # skip_feat[5] =self.multihead(skip_feat[2],skip_feat[3],skip_feat[4],skip_feat[5])
        # print(skip_feat[5].shape)
