...
from ..nn import AttentionGraphCondKernel
...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, attentiongraph=True, dilated=False, norm_layer=None,
                 base_size=520, crop_size=480, mean=[.485, .456, .406],
                 std=[.229, .224, .225], root='~/.encoding/models', **kwargs):
        super(BaseNet, self).__init__()
        self.attentiongraph = attentiongraph
        self.nclass = nclass
        self.aux = aux
        ...
        self.AttentionGraphCondKernel = AttentionGraphCondKernel([512, 1024, 2048], width=512, norm_layer=norm_layer, up_kwargs=up_kwargs) if attentiongraph else None

    def base_forward(self, x,epoch,rank):
        ...
        if self.attentiongraph:
            #aa = self.AttentionGraphCondKernel(c1, c2, c3, c4)
            return c1, c2, c3, self.AttentionGraphCondKernel(c1, c2, c3, c4,epoch,rank=rank)
        else:
            return c1, c2, c3, c4
        return c1, c2, c3, c4
    #### when run test.py check epcoh default frist !!!!!!!!!
    def evaluate(self, x, target=None,rank=9):
        pred = self.forward(x,epoch=100,rank=rank)
