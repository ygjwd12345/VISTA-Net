...
## Email: danxu@robots.ox.ac.uk or danxuhk@gmail.com
## Copyright (c) 2019
##
...
__all__ = ['AttentionGraphCondKernel']
...
class AttentionGatedMSG(nn.Module):
    ...
    # def struc_att(self,att,bs,h,w,iters=1):
    # W=512
    # ch_weights=torch.ones(bs,W,1,1).cuda()
    # # sp_weights=torch.ones(bs,1,h,w).cuda()
    ...
    # return sp_ag_weights*ch_ag_weights*att
    def forward(self, *inputs):
        # input[0] is last scale feature map
        inputs_se = inputs[1]  # the feature map sending message
        inputs_sr = inputs[0]  # the feature map receiving message
        ...
        # sturcure attention
        # attention_map_1=self.struc_att(attention_map_1,bs,h,w)
        # attention_map_2=self.struc_att(attention_map_2,bs,h,w)
        # attention_map_3=self.struc_att(attention_map_3,bs,h,w)
        attention_map_1 = self.sigmoid(attention_map_1)
        attention_map_2 = self.sigmoid(attention_map_2)
        attention_map_3 = self.sigmoid(attention_map_3)
...
class AttentionGraphCondKernel(nn.Module):
    def __init__(self, ms_featmaps, width=512, norm_layer=BatchNorm2d, up_kwargs=None):
        ...
        self.combination_msgs_5 = nn.Sequential(nn.Conv2d(4 * width, 2 * width, kernel_size=1),
                                                norm_layer(2 * width),
                                                nn.ReLU(inplace=True))
    ...
    def forward(self, *inputs):
        ...
        # message passing from scale 3 & 4 to 5
        att_MSG_35, attention_map_35 = self.attention_MSG_35(feats[0], feats[2])
        att_MSG_45, attention_map_45 = self.attention_MSG_45(feats[0], feats[1])
        att_MSG_55, attention_map_55 = self.attention_MSG_55(feats[0], feats[0])

        output_final = self.combination_msgs_5(torch.cat([att_MSG_35, att_MSG_45, att_MSG_55, feats[0]], 1))
