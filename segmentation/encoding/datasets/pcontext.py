...
class ContextSegmentation(BaseDataset):
    BASE_DIR = 'pcontext/VOCdevkit/VOC2010' # need to modify BASE_DIR to your base data root
    NUM_CLASS = 59
    def __init__(self, root='./datasets', split='train',
        ...
        # mask_file='./datasets/pcontext/mask_files/'
        mask_file = os.path.join('./datasets/pcontext/VOCdevkit/VOC2010', self.split+'.pth')
        # modify your mask root
    ...
    def __getitem__(self, index):
        ...
        img_o = cv2.imread(os.path.join(self.detail.img_folder, path))
        if self.mode == 'test':
            if self.transform is not None:
                img = self.transform(img)
            return img, os.path.basename(path), torch.from_numpy(img_o)
